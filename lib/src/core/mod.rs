...
impl Attestation {
    pub async fn evaluate(
        ...
        let tee_evidence_parsed_claims = match verifier.evaluate(&evidence).await {
            Ok(claims) => claims,
            Err(e) => {
        ...
        let opa_input_data = serde_json::to_string(&tee_evidence_parsed_claims)?;
        ...
        let opa_reference_data = match reference_data {
            ...
            None => {
                ...
                let claims_map: HashMap<String, serde_json::Value> =
                    serde_json::from_value(tee_evidence_parsed_claims.clone())?;
        ...
        let attestation_results = AttestationResults {
            ...
            tcb: serde_json::to_string(&tee_evidence_parsed_claims)?,
        };
